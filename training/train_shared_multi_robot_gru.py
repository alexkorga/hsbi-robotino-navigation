"""Pretrain the one-million-parameter shared GRU navigation policy."""
...
from robotino_fleet.learning.gru_policy import GruActorCritic, GruPolicyArchitecture
...
EXPERIMENT_NAME = "shared_multi_robot_gru"
MODEL_PATH = Path("models/shared_multi_robot_gru.zip")
...
PHASES = (
    Phase("open_shared", 500_000, False, 3.0, 0.25, "waypoints", 3e-4, 0.005),
    Phase("factory_shared", 1_250_000, True, 4.0, 0.40, "waypoints", 3e-4, 0.005),
    Phase("factory_conflicts", 1_250_000, True, None, 0.75, "waypoints", 3e-4, 0.005),
)
ROLLOUT_STEPS = 256
SEQUENCE_LENGTH = 32
...
def argument_parser() -> argparse.ArgumentParser:
    """Return the CLI parser for GRU pretraining controls and output paths.

    Returns:
        argparse.ArgumentParser: The CLI parser for GRU pretraining controls and output
            paths.
    """
...
def make_environment(
    ...
    *,
    ...
    """Build monitored shared-policy worlds for curriculum phase.

    worlds sets independent simulations, seed initializes sampling,
    parallel_worlds selects spawned workers, and startup_batch_size
    limits concurrent worker startup. Return the VecMonitor consumed by PPO.
    ...
    Returns:
        VecMonitor: Constructed monitored shared-policy worlds for curriculum phase.
    """
...
def environment_settings(seed: int, phase: Phase) -> LearningEnvironmentSettings:
    """Return three-Robotino learning settings for phase and seed.
    ...
    Returns:
        LearningEnvironmentSettings: Three-Robotino learning settings for phase and seed.
    """
...
def main() -> None:
    """Run GRU curriculum phases and save checkpoints, model, and profile.

    Parses CLI settings, creates each phase environment, trains one shared
    policy across them, and closes resources on completion or failure.
            ...
            try:
                ...
                if trainer is None:
                    shapes = {
                        ...
                        for key, space in environment.observation_space.spaces.items()
                    }
                    ...
                    parameters = sum(
                        value.numel()
                        for value in policy.parameters()
                ...
                print(
                    ...
                    f"{agents} agents, learning rate {learning_rate:g}, "
                ...
                environment.close()
        if trainer is None:
            raise RuntimeError("No GRU curriculum phases were configured")
