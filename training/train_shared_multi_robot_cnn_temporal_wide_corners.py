"""Fine-tune the selected four-frame wide CNN on sustained corner navigation."""
...
EXPERIMENT_NAME = "shared_multi_robot_cnn_temporal_wide_corners"
MODEL_PATH = Path("models/shared_multi_robot_cnn_temporal_wide_corners.zip")
SOURCE_PATH = Path("models/deliverable/shared_multi_robot_cnn_temporal_wide.zip")
...
PHASES = (
    Phase("island_corner_focus", 1_500_000, True, None, 0.0, "island-corners", 1e-4),
    Phase("factory_corner_mix", 1_500_000, True, None, 0.50, "island-corner-mix", 5e-5),
)
...
def environment_settings(seed: int, phase: Phase) -> LearningEnvironmentSettings:
    """Return island-corner curriculum settings for phase and seed.
    ...
    Returns:
        LearningEnvironmentSettings: Island-corner curriculum settings for phase and seed.
    """
...
def argument_parser() -> argparse.ArgumentParser:
    """Return CLI options for wide-CNN corner fine-tuning.

    Returns:
        argparse.ArgumentParser: CLI options for wide-CNN corner fine-tuning.
    """
...
def make_environment(
    ...
    *,
    ...
    """Build monitored multi-Robotino worlds for corner phase.

    worlds sets simulation count, seed randomizes episodes,
    parallel_worlds enables worker processes, and startup_batch_size
    limits launch concurrency. Return the SB3 rollout environment.
    ...
    Returns:
        VecMonitor: Constructed monitored multi-Robotino worlds for corner phase.
    """
...
def main() -> None:
    """Fine-tune a compatible wide CNN on corner and mixed routes.

    Parses CLI settings, checks the required source model/profile, saves
    checkpoints plus final artifact/profile, and closes phase environments.
    ...
    source = project_path(arguments.source)
    ...
    if model is None:
        raise RuntimeError("No fine-tuning phases were configured")
